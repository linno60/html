(function ($) {
    ...
    $('._btn-refresh').click(function (e) {
        ...
        } else if (tab === 'fonts-com')
        {
            var field = $('#fonts-com-parsed');
            var P = {'css': field.val()};
            console.log(P);
            $.post('/elements/submit_fonts_com', P, function (D) {
                location.reload();
            }, 'json');
        } else if (tab === 'fonts-squirel')
        {
            var field = $('#fonts-squirel-parsed');
            var P = {'css': field.val()};
            console.log(P);
            $.post('/elements/submit_fonts_squirel', P, function (D) {
                location.reload();
            }, 'json');
        } else if (tab === 'js')
        {
    ...
    /* --- DELETES --- */
    $('._btn-delete').click(function (e) {
        e.preventDefault();
        var tab = $(this).attr('data-tab');

        if (tab === 'fonts-com') {
            window.location.href = '/elements/delete_fonts_com/';
        } else if (tab === 'fonts-squirel') {
            window.location.href = '/elements/delete_fonts_squirel/';
        }
    });

    /* --- SUBMIT FONTS.COM ZIP FILE --- */
    $('#fonts-com-file-form input[type=file],#fonts-squirel-file-form input[type=file]').change(function () {
    ...
    // Syntax highlighter for text area (fonts-com)
    var CM_object3 = $('#_tab-fonts-com textarea');
    var CodeMirror_fonts_com = CodeMirror.fromTextArea(CM_object3[0], {theme: 'mdn-like'});
    CodeMirror_fonts_com.on('change', function (cm, change) {
        CM_object3.val(cm.getValue());
    });

    // Syntax highlighter for text area (fonts-squirel)
    var CM_object4 = $('#_tab-fonts-squirel textarea');
    var CodeMirror_fonts_squirel = CodeMirror.fromTextArea(CM_object4[0], {theme: 'mdn-like'});
    CodeMirror_fonts_squirel.on('change', function (cm, change) {
        CM_object4.val(cm.getValue());
    });

    // Refresh CM content if the tab was initially hidden
    $('[href="#_tab-fonts-local"], [href="#_tab-js"], [href="#_tab-fonts-com"], [href="#_tab-fonts-squirel"]').click(function () {
    ...
    $('#_tabs_box > div').each(function (key, tab) {
        ...
        if (tname === 'fonts' || tname === 'fonts-com' || tname === 'fonts-squirel' || tname === 'fonts-local' || tname === 'css' || tname === 'js')
            return true;
