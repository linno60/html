<link rel="stylesheet" href="/fonts/font-awesome-4.7.0/css/font-awesome.min.css">
<table class="table">
    <thead>
        <tr class="uppercase">
            <th>guest</th>
            <th>conf</th>
            <th>room</th>
            <th>status</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
    </tbody>
</table>
<table class="table table-bordered table-bordered-td table-striped">
    <thead>
        <tr>
            <th>Name</th>
            <th>Room Total</th>
            <th>Comission</th>
            <th>Comission Amount</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>Direct</td>
            <td>$ 393,80</td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Wholesaler</td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>OTA</td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Corporate Client</td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Travel Agent</td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
    </tbody>
</table>
<table class="table table-bordered-tr table-striped-th">
    <thead>
        <tr>
            <th>
                <input class="check-btn" id="label_1146" type="checkbox" name="show_all_countries">
                <label for="label_1146"></label>
            </th>
            <th>Reservation</th>
            <th>Name</th>
            <th>Surname</th>
            <th>Date Booked</th>
            <th>Room Number(s)</th>
            <th>Check-In</th>
            <th>Check-Out</th>
            <th>Nights</th>
            <th>Total Price</th>
            <th>Status</th>
            <th>Source</th>
            <th>Delete</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>
                <input class="check-btn" id="label_1146" type="checkbox" name="show_all_countries">
                <label for="label_1146"></label>
            </td>
            <td>6379041803</td>
            <td><a href="/">Alona</a></td>
            <td><a href="/">1</a></td>
            <td>07/05/2015</td>
            <td>N/A</td>
            <td>01/03/2015</td>
            <td>07/05/2015</td>
            <td>67</td>
            <td>$ 8716,72</td>
            <td>No Show</td>
            <td>Phone</td>
            <td></td>
        </tr>
        <tr>
            <td>
                <input class="check-btn" id="label_1146" type="checkbox" name="show_all_countries">
                <label for="label_1146"></label>
            </td>
            <td>6379041803</td>
            <td><a href="/">Alona</a></td>
            <td><a href="/">1</a></td>
            <td>07/05/2015</td>
            <td>N/A</td>
            <td>01/03/2015</td>
            <td>07/05/2015</td>
            <td>67</td>
            <td>$ 8716,72</td>
            <td>No Show</td>
            <td>Phone</td>
            <td></td>
        </tr>
        <tr>
            <td>
                <input class="check-btn" id="label_1146" type="checkbox" name="show_all_countries">
                <label for="label_1146"></label>
            </td>
            <td>6379041803</td>
            <td><a href="/">Alona</a></td>
            <td><a href="/">1</a></td>
            <td>07/05/2015</td>
            <td>N/A</td>
            <td>01/03/2015</td>
            <td>07/05/2015</td>
            <td>67</td>
            <td>$ 8716,72</td>
            <td>No Show</td>
            <td>Phone</td>
            <td></td>
        </tr>
    </tbody>
</table>
<table class="table table-bordered table-bordered-tr table-bordered-td table-striped table-hover">
    <thead>
        <tr>
            <th>Active/Inactive</th>
            <th>Name</th>
            <th>Last Closing Balance</th>
            <th>Times Opened</th>
            <th>Last Opened</th>
            <th>Last Closed</th>
            <th>Last Opened By</th>
            <th></th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td></td>
            <td>Alona</td>
            <td>$689,00</td>
            <td>2</td>
            <td>17/02/2017 09:04</td>
            <td>17/02/2017 09:04</td>
            <td>SYSTEM</td>
            <td><a href="/" class="btn btn-default edit-notes"><i class="fa fa-pencil"></i></a></td>
        </tr>
        <tr>
            <td></td>
            <td>Alona</td>
            <td>$689,00</td>
            <td>2</td>
            <td>17/02/2017 09:04</td>
            <td>17/02/2017 09:04</td>
            <td>SYSTEM</td>
            <td><a href="/" class="btn btn-default edit-notes"><i class="fa fa-pencil"></i></a></td>
        </tr>
        <tr>
            <td></td>
            <td>Alona</td>
            <td>$689,00</td>
            <td>2</td>
            <td>17/02/2017 09:04</td>
            <td>17/02/2017 09:04</td>
            <td>SYSTEM</td>
            <td><a href="/" class="btn btn-default edit-notes"><i class="fa fa-pencil"></i></a></td>
        </tr>
        <tr>
            <td></td>
            <td>Alona</td>
            <td>$689,00</td>
            <td>2</td>
            <td>17/02/2017 09:04</td>
            <td>17/02/2017 09:04</td>
            <td>SYSTEM</td>
            <td><a href="/" class="btn btn-default edit-notes"><i class="fa fa-pencil"></i></a></td>
        </tr>
        <tr class="warning">
            <td></td>
            <td>Alona</td>
            <td>$689,00</td>
            <td>2</td>
            <td>17/02/2017 09:04</td>
            <td>17/02/2017 09:04</td>
            <td>SYSTEM</td>
            <td><a href="/" class="btn btn-default edit-notes"><i class="fa fa-pencil"></i></a></td>
        </tr>
    </tbody>
</table>